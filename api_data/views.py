...
class api_custom_field(ListAPIView):
    ...
    def post(self, request):
        serializer = CustomFieldSerializer(data = request.data, context = {'agent_id':request.user})
    ...
    def put(self, request):
        try:
            custom_field_obj = custom_field.objects.get(id = request.data["id"], agent_id = request.user)
            serializer = CustomFieldSerializer(custom_field_obj, data = request.data, partial = True )
            if serializer.is_valid():
                print(serializer.validated_data)
                created_data = serializer.save()
                # send success response
